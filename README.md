...
首先你需要一个脚本管理器，然后在[洛谷](https://www.luogu.com.cn/article/ugc80dim/edit)复制脚本代码，并粘贴为新脚本。或者您也可以前往 [GreasyFork](https://greasyfork.org/zh-CN/scripts/582602-%E6%B4%9B%E8%B0%B7%E5%8F%96%E5%85%B3%E6%8F%90%E9%86%92%E5%99%A8) 安装。
	...
	get_btn.addEventListener("click", async () => {
		...
		async function get_all_followers(uid)
		{
			...
			if (cdata.success)
			{
				...
				let nc = 0, success = true, list = [];
				get_card_status.innerText = `loading... 0 / ${count}`;
				for (let i = 1; i <= pages; i++)
				{
					if (cls) return null;
					get_followers(uid, i).then((res) => {
		...
		function display_followers(list)
		{
			for (const user of list)
			{
				let hook_color = "var(--lfe-color--green-3)";
				if (user.ccfLevel >= 6) hook_color = "var(--lfe-color--blue-3)";
				if (user.ccfLevel >= 8) hook_color = "var(--lfe-color--gold-3)";

				const ccf_hook = `
					<svg class="svg-inline--fa fa-badge-check" data-prefix="fad" data-icon="badge-check" role="img" viewBox="0 0 512 512" style="--fa-primary-color: #fff; --fa-secondary-color: ${hook_color}; --fa-secondary-opacity: 1;">
				...
				`;
				insert_el(get_card, "div", html, "", "fcker-user");
			}
		}
